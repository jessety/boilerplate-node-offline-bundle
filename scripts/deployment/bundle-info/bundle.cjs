...
function describe(bundle) {
  ...
  return `${date.toLocaleDateString()}, ${name} ${cyan(version)} with node ${cyan(
    node
  )} and npm ${cyan(npm)} on ${cyan(platform)}-${cyan(release)}.`;
}
...
async function compare(bundle, current) {
  ...
  if (bundle.package.version !== version) {
    console.warn(
      `${colors.yellow('WARNING')}: Bundle was created with ${bundle.package.name} ${colors.cyan(
        bundle.package.version
      )}, but this is ${colors.green(version)}.`
    );
  }
  ...
  if (bundle.os.platform !== platform || bundle.os.release !== release) {
    console.warn(
      `${colors.yellow('WARNING')}: Bundle was created on ${colors.cyan(
        bundle.os.platform
      )}-${colors.cyan(bundle.os.release)} but this is ${colors.green(platform)}-${colors.green(
        release
      )}.`
    );
  }
  ...
  if (bundle.package.version !== version || bundle.node !== node || bundle.npm !== npm) {
    console.error(
      `${colors.red(
        'ERROR'
      )}: Bundle does not match the version of the ${name}, node, or npm. This may cause bundled installation to fail. \nPlease ensure bundle is created with the same version as the installation target.`
    );
...
async function read() {
  try {
    ...
    console.warn(
      `${colors.yellow(
        'WARNING'
      )}: Bundle info file either not found or corrupt. If internet access is available, this is not an issue. For offline installation, please ensure this package is bundled with its dependencies prior to being transferred to the installation target.`
    );
